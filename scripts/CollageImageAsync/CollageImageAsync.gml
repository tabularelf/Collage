        ...
        repeat(array_length(_asyncList)) {
            if (_id == _asyncList[_j].spriteID) {
                if (_status >= 0) {
					...
					__CollageTrace(_asyncList[_j].name + " loaded from the internet (" + string (_fileName) + ")!");
                } else {
                    __CollageTrace(_asyncList[_j].name + " failed to load from (" + string (_fileName) + ")! Error status: " + string(_status));
                }
                ...
                if (array_length(_asyncList) == 0) {
					...
					_texPage.__status = __CollageStatus.READY;
                    if (_texPage.__state == __CollageStates.NORMAL) _texPage.builder.__build();
